...
pub struct Crazyflie {
    _link_context: Rc<crazyflie_link::LinkContext>,
...
impl Crazyflie {
    pub async fn connect(uri: String) -> Result<Crazyflie, JsValue> {
        ...
        Ok(Crazyflie {
            ...
            _link_context: Rc::new(link_context),
            commander: Commander {
...
#[wasm_bindgen]
pub struct LogBlock {
    block: Rc<Mutex<crazyflie_lib::subsystems::log::LogBlock>>,
}
...
impl LogBlock {
    ...
    pub async fn start(self, period_ms: usize) -> Result<LogStream, JsValue> {
        ...
        let period = crazyflie_lib::subsystems::log::LogPeriod::from_millis(period_ms as u64)
            .map_err(|e| format!("{:?}", e))?;
...
pub struct LogStream {
    stream: Rc<Mutex<crazyflie_lib::subsystems::log::LogStream>>,
}
